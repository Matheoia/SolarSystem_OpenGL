...
int main()
{
    ...
    ////////// INSTANCIATION DES ETOILES, DU SOLEIL, DES ASTRES, .obj DES ORBITES ET POSITIONS //////////
    ...
    while(glfwGetKey(window, GLFW_KEY_ESCAPE) != GLFW_PRESS && !glfwWindowShouldClose(window)){
        ...
        ////////// ROTATION ET ORBITE DES PLANETES //////////
        ////////// PUIS ENVOIE DE LA MATRICE MODEL SCALE POUR CHAQUE .OBJ //////////
        /// ////////// ET DESSINER L'OBJET //////////
        //MERCURE
        mercure.rotationAngles.y = interface.getSpeedCoef() * currentTime/59/4;
        mercure.position.x = -20.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 88.0f);
        mercure.position.z = -20.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 88.0f);
        shader.setUniformMat4f("model", glm::scale(mercure.getModelMatrix(), glm::vec3(0.40f)));
        renderer.Draw(va, mercure, shader);

        //VENUS
        venus.rotationAngles.y = interface.getSpeedCoef() * currentTime/243/4;
        venus.position.x = 30.0f *  cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 225.0f);
        venus.position.z = 30.0f *  sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 225.0f);
        shader.setUniformMat4f("model", glm::scale(venus.getModelMatrix(), glm::vec3(0.94f)));
        renderer.Draw(va, venus, shader);

        //TERRE
        terre.rotationAngles.y = interface.getSpeedCoef() * currentTime/4;
        terre.position.x = -40.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 365.25f);
        terre.position.z = -40.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 365.25f);
        shader.setUniformMat4f("model", terre.getModelMatrix());
        renderer.Draw(va, terre, shader);

        //MARS
        mars.rotationAngles.y = interface.getSpeedCoef() * currentTime/1.025/4;
        mars.position.x = 55.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 687.0f);
        mars.position.z = 55.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 687.0f);
        shader.setUniformMat4f("model", glm::scale(mars.getModelMatrix(), glm::vec3(0.53f)));
        renderer.Draw(va, mars, shader);

        //JUPITER
        jupiter.rotationAngles.y = interface.getSpeedCoef() * currentTime/0.42/4;
        jupiter.position.x = -160.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 4328.9f + 0.9f);
        jupiter.position.z = -160.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 4328.0f + 0.9f);
        shader.setUniformMat4f("model", glm::scale(jupiter.getModelMatrix(), glm::vec3(10.97f)));
        renderer.Draw(va, jupiter, shader);

        //SATURNE
        saturne.rotationAngles.y = interface.getSpeedCoef() * currentTime/0.4/4;
        saturne.position.x = 280.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 10752.9f - 2.0f);
        saturne.position.z = 280.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 10752.9f - 2.0f);
        shader.setUniformMat4f("model", glm::scale(saturne.getModelMatrix(), glm::vec3(9.14f)));
        renderer.Draw(va, saturne, shader);

        anneauSaturne.rotationAngles.y = interface.getSpeedCoef() * currentTime/0.4/4;
        anneauSaturne.position.x = 280.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 10752.9f - 2.0f);
        anneauSaturne.position.z = 280.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 10752.9f - 2.0f);
        shader.setUniformMat4f("model", glm::scale(anneauSaturne.getModelMatrix(), glm::vec3(9.14f)));
        renderer.Draw(va, anneauSaturne, shader);

        //URANUS
        uranus.rotationAngles.y = interface.getSpeedCoef() * currentTime/0.7/4;
        uranus.position.x = -580.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 30660.0f - 1.07f);
        uranus.position.z = -580.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 30660.0f - 1.07f);
        shader.setUniformMat4f("model", glm::scale(uranus.getModelMatrix(), glm::vec3(3.98f)));
        renderer.Draw(va, uranus, shader);

        //NEPTUNE
        neptune.rotationAngles.y = interface.getSpeedCoef() * currentTime/0.66/4;
        neptune.position.x = 880.0f * cos(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 61243.35f + 1.07f);
        neptune.position.z = 880.0f * sin(interface.getSpeedCoef() * currentTime * 2.0f * glm::pi<float>() / 61243.35f + 1.07f);
